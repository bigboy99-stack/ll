...
const cp = Array.from(document.querySelectorAll(".copy"));
const ema = Array.from(document.querySelectorAll(".ema"));
...
cp[0].onclick = () => {
    ema[0].select();
    ema[0].setSelectionRange(0, 99999);
    navigator.clipboard.writeText(ema[0].value);
    alert("email address copied: " + ema[0].value);
}

cp[1].onclick = () => {
  none.select();
  none.setSelectionRange(0, 99999);
  navigator.clipboard.writeText(none.value);
  alert("Phone Number Cpoied: " + none.value);
}
